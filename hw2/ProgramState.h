...
public:
    ...
    int at(std::string variable);
    bool inMap(std::string variable);
    void changeVal(std::string variable, int value);
...
private:
	...
	std::map<std::string, int> vars; //map to keep track of variables
	
	std::stack<int> sub; //stack to keep track of GOSUB and RETRURN
